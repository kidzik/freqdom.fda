...
#' @param Ndpc an integer \eqn{\in\{1,\ldots, d\}} with \eqn{d} = F\eqn{\$}basisX\eqn{\$}nbasis. It is the number of dynamic principal components to be computed. By default it is set equal to \eqn{d}.
...
#' * operators & an array. Each matrix in this array has dimension \eqn{\code{Ndpc}\times d} and is assigned to a certain lag.
...
#' * basisX F\eqn{\$}basis, hence an object of class ``basisfd".
#' * correspondence the correspondence matrix \eqn{B}: all scalar products between basis functions.
