...
#' 
#' @title From a given spectral density operator the dynamic principal component filter sequences are computed.
...
#' @param Ndpc an integer \eqn{\in\{1,\ldots, d\}} with \code{d = F$basisX$nbasis}. It is the number of dynamic principal components to be computed. By default it is set equal to \eqn{d}.
...
#' * \code{operators} - an array. Each matrix in this array has dimension \eqn{\code{Ndpc}\times d} and is assigned to a certain lag.
...
#' * \code{basisX} - \code{F$basis}, hence an object of class \code{\link{basis.fd}}.
#' * \code{correspondence} - the correspondence matrix \eqn{B}: all scalar products between basis functions.
